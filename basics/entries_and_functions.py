...
img = tkinter.PhotoImage(file='icons/entries.png')
...
def make_label():
    ...
    text = tkinter.Label(output_frame, text=text_entry.get(), bg='#ccc')
    text.pack()
...
def count_up(number):
    '''Count up on the app'''
    global value
    text = tkinter.Label(output_frame, text=number, bg='#ccc')
    text.pack()

    value+=1

# Define Frames
input_frame = tkinter.Frame(root, bg='#f3f3f3', width=500, height=100)
output_frame = tkinter.Frame(root, bg='#ccc', width=500, height=400)
